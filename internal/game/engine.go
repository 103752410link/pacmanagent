...
type GameState struct {
	...
	Phase                string `json:"phase"` // "player_turn" | "ghost_turn"
...
type Engine struct {
	...
	ghostMoves     map[string]string      // ghostID -> direction for this turn
...
// SubmitGhostMove adds a move for a ghost
func (e *Engine) SubmitGhostMove(agentID, direction string) (bool, string) {
	...
	e.ghostMoves[agentID] = direction
	e.timeoutAt = time.Now().Add(time.Duration(e.timeoutSeconds) * time.Second)

	// Check if all ghosts submitted
	allSubmitted := true
	for _, id := range e.ghostIDs {
		if _, ok := e.ghostMoves[id]; !ok {
			allSubmitted = false
			break
		}
	}

	if allSubmitted || len(e.ghostMoves) >= len(e.ghostIDs) {
		// Execute ghost phase
...
func (e *Engine) CheckTimeout() {
	...
	if !e.playerAgentRegistered {
		...
		// Ghost phase — auto-advance
		for _, id := range e.ghostIDs {
			if _, ok := e.ghostMoves[id]; !ok {
				e.ghostMoves[id] = "stay"
			}
		}
		e.executeGhostPhaseLocked()
		e.checkCollisionsLocked()
	...
	if e.phase == 0 {
		...
		// Ghost timeout - fill missing ghost moves with "stay"
		for _, id := range e.ghostIDs {
			if _, ok := e.ghostMoves[id]; !ok {
				e.ghostMoves[id] = "stay"
			}
...
func (e *Engine) GetState() GameState {
	...
	ghostMovesSubmitted := len(e.ghostMoves)
	...
	return GameState{
		...
		GhostMovesNeeded:     len(e.ghostIDs),
